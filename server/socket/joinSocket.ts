  ...
  return (config: IConfig) => {
    ...
    clients.forEach((clientId) => {
      ...
      socket.emit(ACTIONS.ADD_PEER, {
        peerId: clientId,
        createOffer: true, // так как оффер должна создавать только та сторона, которая
        // подключается к комнате. Те кто уже в комнате, они будут у
        // себя просто добавлять нового пользователя к пирам
      })

      /*
      тот кто создал комнату он ничего не будет делать, так как в комнате никого еще нет. 
      А когда зайдет второй, то первый уже находящийся в комнате получит новый пир, 
      а тот кто зашел, тот создаст оффер для тех кто уде в комнате
      */
    })
